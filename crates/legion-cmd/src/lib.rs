...
//! `Decision` set, the closed six-reason `ProxyReason` set, the facts `route`
//! extracts from a command, and the context a caller supplies to it.
//!
...
pub use decision::{
    Context, ContractError, Decision, DenyDetails, Facts, Lookup, ManagedTarget, ProxyReason,
    Routed, ToolCall,
};
