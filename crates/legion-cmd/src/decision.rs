...
pub enum ContractError {
    ...
    /// A string outside the closed six-member set was parsed as a
...
impl fmt::Display for ManagedTarget {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(&self.0)
    }
}

/// The closed set of six proxy reasons (FR-CMD-004). No other value can be
/// constructed: [`TryFrom<&str>`] rejects anything outside the six, and the
/// custom [`Deserialize`] impl below routes through that same check, so a
/// `ProxyReason` read from JSON is held to the identical set as one built in
/// Rust. Serialized forms use the six kebab-case names.
