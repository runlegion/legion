...
fn run() -> error::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Consult {
            ...
            until,
            on,
        } => cli::memory::handle_consult(context, symbol, limit, json, tier, since, until, on)?,
        Commands::Post {
            repo,
            text,
            transcript,
        ...
        Commands::Inbox { repo, split } => cli::inbox::handle_inbox(repo, split)?,
        Commands::Deliver { action } => cli::inbox::handle(action)?,
        ...
        Commands::CmdCheck {
            hook,
            deny_patterns,
        } => cli::cmd_check::handle_cmd_check(hook, deny_patterns)?,
