...
fn run() -> error::Result<()> {
    ...
    match cli.command {
        Commands::Reflect { repo, text, transcript, domain, whoami, tags, follows, force, dedupe_mode } => cli::memory::handle_reflect(repo, text, transcript, domain, whoami, tags, follows, force, dedupe_mode)?,
        Commands::Forget { id, repo } => cli::memory::handle_forget(id, repo)?,
        Commands::Recall { repo, context, limit, latest, preview, cosine_only, min_score, domain, archives, include_archives } => cli::memory::handle_recall(repo, context, limit, latest, preview, cosine_only, min_score, domain, archives, include_archives)?,
        Commands::Similar { id, limit, cross_repo, min_score, preview, json } => cli::memory::handle_similar(id, limit, cross_repo, min_score, preview, json)?,
        Commands::Consult { context, symbol, limit, json } => cli::memory::handle_consult(context, symbol, limit, json)?,
        Commands::Post { repo, text, transcript, domain, tags, follows } => cli::signal::handle_post(repo, text, transcript, domain, tags, follows)?,
        Commands::Signal { repo, to, verb, status, note, details, follows, domain, tags } => cli::signal::handle_signal(repo, to, verb, status, note, details, follows, domain, tags)?,
        ...
        Commands::Bullpen { repo, count, signals, musings, archive, archived, include_stale, include_resolved } => cli::signal::handle_bullpen(repo, count, signals, musings, archive, archived, include_stale, include_resolved)?,
        Commands::Index { repo, file, status, logs, follow, lines, banner, json } => cli::index_cmd::handle_index(repo, file, status, logs, follow, lines, banner, json)?,
        ...
        Commands::Audit { repo, action, limit, json } => cli::ops::handle_audit(repo, action, limit, json)?,
        ...
        Commands::Verify { repo: _repo, card, verdicts_file } => cli::verify::handle_verify(card, verdicts_file)?,
        ...
        Commands::Usage { session, since, today: _today, by_session, by_repo, json } => cli::ops::handle_usage(session, since, by_session, by_repo, json)?,
        Commands::Health { history, all_hosts, json } => cli::ops::handle_health(history, all_hosts, json)?,
    }
