...
| `pre-bash-ls.sh` | PreToolUse / Bash | inject only -- never denies | ADVISORY by explicit design | Nothing, and nothing is needed: it nudges an `ls` of an indexed repo's tree toward `legion sym tree`/`sym list` and always allows the command. `ls` is not a lossless equivalent of `sym tree` (metadata, non-source files), so it injects rather than blocks; the cost of an escape is one injected paragraph or an uncounted listing (#976). |
