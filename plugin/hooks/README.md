...
This is REQUIRED by design, not a defect awaiting a patch. The header of
`no-git-push.sh` (retired by the legion-cmd cutover, #1236; the reason holds
unchanged for the `legion-cmd.sh` adapter that replaced it) stated the reason:
...
**The legion-cmd cutover (#1236) retired the twelve command hooks below.**
Their rows stay, because the verdict and the enforcing layer belong to the
class of command, not to the script: each row now names the policy rules in
...
|---|---|---|---|---|
| `no-git-push.sh` | PreToolUse / Bash | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rule `git-push-to-legion` (family `git push`). Was: rewrite to `legion push`; deny untranslatable flags | **MUST-BE-TOTAL** | **Partially covered.** `.githooks/pre-push` is the only layer that sees a script's push (`git config core.hooksPath` resolves to `.githooks` in this checkout and its worktrees, checked 2026-08-04), but it **exempts `main`/`master` by name** (`.githooks/pre-push:52-55` `[CODE]`), reads the CWD's current branch rather than the refs on stdin, and **exits 0 when the reviewer is unavailable** (`.githooks/pre-push:87-90` `[CODE]`; observed firing on the push of this very commit `[REPRO]`, see Evidence 3). `REFUSED_BRANCHES` (`src/cli/push.rs:20` `[CODE]`) only fires when something calls `legion push` -- a script calling `git push` never reaches it. Remote branch protection: `[UNVERIFIED]` here; the 2026-08-04 incident is direct evidence that a push to `refs/heads/main` was accepted by the remote at that time `[INCIDENT]`. **Fix in flight: #859.** |
| `no-git-commit.sh` | PreToolUse / Bash | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rules `git-commit-to-legion` and `git-commit-without-message` (family `git commit`). Was: rewrite to `legion commit`; deny untranslatable flags | **Split.** Audit row + message-convention enforcement: **MUST-BE-TOTAL, no enforcing layer today.** Signing: ADVISORY | `.githooks/pre-commit` is the only git-layer hook on this path, and it runs a `claude -p` code-quality review, nothing else (`.githooks/pre-commit:78-100` `[CODE]`) -- it does not check the `<type>(<scope>): <summary>` subject shape, emoji, or write anything resembling `legion commit`'s audit row, and it fails open exactly like `pre-push` when the reviewer is unavailable (`.githooks/pre-commit:88-91` `[CODE]`, same weakness as Evidence 3). A script-shaped `git commit` therefore lands with **none** of `legion commit`'s guarantees except signing, which is ADVISORY here because it does not depend on this hook at all: git signs a commit whenever `commit.gpgsign`/`user.signingkey` are configured, script or not -- `legion commit`'s signer preflight only makes a locked signer fail with a named reason instead of a cryptic one, it adds no new signing capability. The audit-row and convention-enforcement gaps have no fix in flight; not filed as an issue by this change (out of scope for an implementer agent -- see PR body). |
| `no-direct-db.sh` | PreToolUse / Bash | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rules `sqlite3-legion-db` and `sqlite3-other-db` (family `sqlite3`). Was: deny any command naming `legion.db` | **Split.** Reads: ADVISORY. **Writes: MUST-BE-TOTAL** | **Nothing enforces the write case.** `legion.db` is a plain user-owned SQLite file; a script running `sqlite3` writes to it with no barrier. There is no `PRAGMA user_version` guard, no integrity probe, and `PRAGMA foreign_keys` is not enabled globally (`src/db/kanban.rs:109`, `src/db/wake.rs:155` `[CODE]`), so the schema does not constrain an out-of-band writer either. `legion health` samples machine load, not database integrity (`src/health.rs:7-26` `[CODE]`). The hook's own header already concedes an in-band gap (`ATTACH` from an interactive `sqlite3` prompt) and calls the mitigation "cultural, not technical". Since the cutover the Claude-layer guard is narrower than the retired hook: the policy's argument predicates compare whole arguments, so `sqlite3` or `cp` against the store by path is allowed (hook_cases.json rows `db-sqlite3-legion-db-by-path`, `db-copy-legion-db`); **filed as #1311.** **No total layer exists today; filed as #864 -- see the section below.** |
| `no-gh.sh` | PreToolUse / Bash | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rules `gh-unmapped`, `gh-pr-to-legion`, `gh-issue-to-legion`, `gh-run-to-pr-checks`, `gh-pr-diff`, `gh-pr-view-to-legion`, `gh-pr-checks-to-legion`, `gh-pr-list-to-legion`, `gh-issue-view-to-legion`, `gh-issue-list-to-legion` (the `gh` families). Was: deny, with a translated `legion` command | **Split.** Read/query verbs: ADVISORY. **`gh pr merge` and ref-writing `gh api`: MUST-BE-TOTAL** | **Advisory half:** nothing, by design. `legion` implements a subset of `gh` with different ergonomics, so a rewrite would silently drop flags (#828) and a total block would refuse work legion cannot do; the cost of an escape is a missing `legion audit` row, not a broken repo. **Total half: nothing enforces it, and the git layer cannot.** `gh pr merge` lands code on the default branch outside the queue, and `gh api -X PATCH /repos/:o/:r/git/refs/heads/main` rewrites the exact ref the 2026-08-04 incident rewrote -- neither shells out to git, so `.githooks/pre-push` never runs and `REFUSED_BRANCHES` is never consulted. Only remote branch protection remains, `[UNVERIFIED]` above. **Filed as #866.** |
| `no-local-memory.sh` | PreToolUse / Write, Edit, MultiEdit | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rules `write-auto-memory`, `edit-auto-memory`, `multiedit-auto-memory`. Was: deny writes under `~/.claude/projects/*/memory/` | ADVISORY | Nothing. A file written there is invisible to other agents and repos -- the cost of an escape is knowledge stranded in one session, not damage. See "matcher gaps" below: this one does not need a script to escape. |
| `no-harness-explore.sh` | PreToolUse / Agent, Task | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rules `agent-explore-to-legion`, `agent-untyped`, `task-explore-to-legion`, `task-untyped`. Was: rewrite `subagent_type: Explore` to `legion:legion-explore` via `updatedInput` | ADVISORY | Nothing, and nothing is needed: subagents are spawned through the Agent/Task tool, so there is no shell form of this call for a script to carry. The child-process boundary does not apply. |
| `pre-grep.sh` | PreToolUse / Grep, Glob | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rules `grep-tool-to-sym` and `glob-tool-to-sym`, which route to the `find-content` / `find-file` sym jobs. Was: deny when `sym def` resolves the pattern locally; else inject | ADVISORY | Operator `permissions.deny` is the mandatory tier (`docs/decisions/2026-06-02-grep-blocking-is-operator-permissions.md`) -- but it is Claude-layer too, so a script-shaped search escapes it identically `[REPRO]`. Advisory either way: this guard controls token cost and routing, not correctness. |
| `pre-bash-grep.sh` | PreToolUse / Bash | **Retired (#1236)** -- replaced by `legion-cmd.sh` with the `*-to-sym` policy rules of the search families (`grep`, `rg`, `egrep`, `fgrep`, `ag`, `ack`, `git grep`, the compressed-file greps, `find`, `fd`, `git ls-files`), which route to the `find-content` / `find-file` sym jobs. Was: deny when `sym def` resolves the pattern locally; else inject | ADVISORY | Same as `pre-grep.sh`. Its own header already describes itself as "SOFT FALLBACK GUIDANCE". |
| `pre-read-sym.sh` | PreToolUse / Read | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rules `read-source-unbounded` and `read-source-oversized-limit`. Was: deny unbounded Reads of large indexed source files | ADVISORY | Nothing, correctly: it is a cost guard with a telemetried `LEGION_BYPASS_READ=1` escape by design. |
| `pre-script-search.sh` | PreToolUse / Bash, Write, Edit | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rules `write-script-tree`, `write-script-file`, `write-script-content`, `write-script-subprocess-grep`, `edit-script-tree`, `edit-script-file`, `edit-script-content`, `edit-script-subprocess-grep`, and for Bash the sym jobs reached through the `script_carriers` and `interpreters` entries (`find-content`, `script-walk-*`, `script-glob-*`, `script-grep-subprocess`). Was: inject only -- never denies | ADVISORY by explicit design | Nothing. Its header states the posture: "INJECT, NEVER DENY. A script that walks a tree may also do real work" |
| `pre-bash-ls.sh` | PreToolUse / Bash | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rule `ls-structure-note` (family `ls`). Was: inject only -- never denies | ADVISORY by explicit design | Nothing, and nothing is needed: it nudges an `ls` of an indexed repo's tree toward `legion sym tree`/`sym list` and always allows the command. `ls` is not a lossless equivalent of `sym tree` (metadata, non-source files), so it injects rather than blocks; the cost of an escape is one injected paragraph or an uncounted listing (#976). |
| `recall-first.sh` | PreToolUse / WebFetch, WebSearch | **Retired (#1236)** -- replaced by `legion-cmd.sh` with policy rules `webfetch-recall-first` and `websearch-recall-clamped`. Was: inject only -- never denies | ADVISORY | Nothing needed. |
...
1. **Direct writes to `legion.db` have no enforcing layer at all.** Not the
   ...
   Found in #860; filed as #864. The `legion-cmd.sh` policy that replaced it
   (#1236) does not yet refuse the store by path; filed as #1311.
...
Both guards below are retired (#1236); the gaps are kept as the record of
what the legion-cmd policy had to cover. These are guards whose *matcher*
misses shapes it means to catch. They are not
