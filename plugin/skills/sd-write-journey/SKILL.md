...
2. Draft against the live schema: resolve by `"x-doc-type": "journey"` from
   ...
   the contract. Today: `meta` requires `title`, `persona`, `scenario`, `goal`; each
   phase requires `number`, `title`, `emotional_start`, `emotional_end`, and `rows` with
   `actions`, `thoughts`, `emotions`, `touchpoints` (plus optional `pain_points` and
   `opportunities`).
3. Fill with traced content:
   ...
   - Every low point in the curve traces to a painmatrix theme. A killed pain must not
     reappear as a dip -- that is the whole point of having killed it.
   - High points trace to value exchanges the ecosystem grounds; no delight the service
     ...
     the thesis's direction supports it.
4. Validate, then create:

```
legion document validate --schema <schema-id> --file journey.json
legion document create --doc-type journey --owner <agent> --surface <repo> --from journey.json
```
