...
3. Fill with traced content only; the trace is structural, not prose:
   - `identity` requires `description`, `mental_model`, and `quote` -- the quote is the
     actor's voice from the evidence, not an invented line.
   ...
   - `frustrations` are `{text, pain_theme}` items where `pain_theme` is the painmatrix
     theme id (T1, T2, ...) -- the trace is a field the schema carries. A killed pain is
     not a frustration; it stays out.
   ...
   - `meta` requires `set` and `actor` alongside `title`; `actor` must match the
     ecosystem's actor name, and once this persona lands, the ecosystem's
     `actors.primary[].persona` (null until authored) can be revised to name it.
4. Validate, then create:

```
legion document validate --schema <schema-id> --file persona.json
legion document create --doc-type persona --owner <agent> --surface <repo> --from persona.json
```
